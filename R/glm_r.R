...
glm_r = function(formula, family = gaussian, data, conf.level = 0.95,
  ...
  RVAL <- list(coefficients = apply(as.matrix(coefficients), 1:2, as.numeric), n = n,
               ...
               call = match.call(),
               null.deviance = mod$null.deviance, deviance = mod$deviance, aic=mod$aic,
               family = list(family=dist, link=link)
               )
...
#
#
# mod = glm(y~x1+x5, df, family=binomial('logit'))
# tictoc::tic()
# boot.pval::boot_summary(mod, type='bca', R=10000)
# tictoc::toc()
